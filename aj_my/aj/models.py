...
class House(BaseModel, db.Model):
    ...
    # 房屋的设施
    facilities = db.relationship("Facility", secondary=ihome_house_facility, backref='house_facility')
...
class Order(BaseModel, db.Model):
    ...
    order_num = db.Column(db.String(100), nullable=False)  # 订单编号
    status = db.Column(
        db.Enum(
            "WAIT_ACCEPT",  # 待接单,
            "WAIT_PAYMENT",  # 待支付
    ...
    reject_reason = db.Column(db.Text)  # 拒单原因
