...
$(document).ready(function () {
   ...
   $.ajax({
       ...
       success:function (data) {
            if(data.code == 1014){
                $('#real-name').attr('placeholder', data.authData.id_name);
                $('#id-card').attr('placeholder', data.authData.id_card);
    ...
    function check () {
            ...
            if (real_name && id_card){
                if (!checkIDCard(id_card)){
                    $('.error-msg').html('<i class="fa fa-exclamation-circle"></i>身份证号码格式错误');
                    $('.error-msg').show();
                    return false
                }
                return true
    ...
    $('#form-auth').submit(function (e) {
        ...
        if(check()){
            ...
            type: 'POST',
            dataType: 'json',
            data: {'id_name': real_name, 'id_card': id_card},
            success:function (data) {
                // console.log(data)
                if (data.code == 200) {
                    alert(data.msg);
